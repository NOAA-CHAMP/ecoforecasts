function y=cor30a_vectorized(u,us,ts,t,Qs,Q,Rs,Rl,rain,zi,P,zu,zt,zq,lat,jcool,jwave,twave,hwave)
...
% SAMPLE CALL:
% cor30a_vectorized(5.5,0,28.7,27.2,24.2,18.5,141,419,0,600,1010,15,15,15,0,1,1,5,1);
%  u  %wind speed (m/s)  at height zu (m)
%  us %surface current speed in the wind direction (m/s)
%  ts %bulk water temperature (C) if jcool=1, interface water T if jcool=0  
%  t  %bulk air temperature (C), height zt
%  Qs %bulk water spec hum (g/kg) if jcool=1, ...
%  Q  %bulk air spec hum (g/kg), height zq
%  Rs  %downward solar flux (W/m^2)
%  Rl  %downard IR flux (W/m^2)
%  rain  %rain rate (mm/hr)
%  zi  %PBL depth (m)
%  P  %Atmos surface pressure (mb)
%  zu  %wind speed measurement height (m)
%  zt  %air T measurement height (m)
%  zq  %air q measurement height (m)
%  lat  %latitude (deg, N=+)
%  jcool  %implement cool calculation skin switch, 0=no, 1=yes
