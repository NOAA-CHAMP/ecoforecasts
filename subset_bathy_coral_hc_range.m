1;
%% SCRIPT subset_bathy_coral_hc_range.m
%
...
%
% Last Saved Time-stamp: <Thu 2017-05-04 22:41:28 Eastern Daylight Time gramer>
...
if ( ~exist('FRT_depth_and_beta_92m_NANMEAN_3_3_4_hc_range_coral_and_hard_bottom.mat','file') )
...
tic,
  ...
  save('FRT_depth_and_beta_92m_NANMEAN_3_3_4_hc_range_coral_and_hard_bottom.mat',...
...
if ( ~exist('FRT_depth_and_beta_92m_hc_range_coral_and_hard_bottom.mat','file') )
disp('Full - RfHd - Hit Enter to continue...'); pause;
tic,
  h=[]; bet=[]; LON=[]; LAT=[]; clear lon lat h bet LON LAT
  load('FRT_depth_and_beta_92m_hc_range.mat');
  [LON,LAT] = meshgrid(lon,lat);
  inix=[];
  disp(numel(shps));
  dlat = 1.1*abs(lat(1)-lat(2));
  ...
  disp(numel(find(~isnan(h))));
  save('FRT_depth_and_beta_92m_hc_range_coral_and_hard_bottom.mat',...
       'lon','lat','h','bet','ang','hcrng');
  clear inix ix shpix;
toc,
end;

if ( ~exist('FRT_depth_and_beta_92m_hc_range_coral.mat','file') )
...
tic,
  ...
  disp(numel(find(~isnan(h))));
  save('FRT_depth_and_beta_92m_hc_range_coral.mat',...
       'lon','lat','h','bet','ang','hcrng');
  clear inix ix shpix;
toc,
end;
