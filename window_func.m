...
%
% Last Saved Time-stamp: <Sun 2018-02-25 20:32:54 Eastern Standard Time gramer>


    newts = [];
    newdts = [];

    % NOTE: Assumes a FIXED fundamental sampling rate (should be sanity checked) 
    hrs_per_sample = median(diff(dts))*24;
    ...
    switch ( lower(func) )
     ...
     case {'lowpass', 'lopass', 'lp'},
      ...
      [B, A] = butter(filterOrder, (nyq/nhrs), 'low');
     ...
     otherwise,
      ...
      % First, catch simple case: NHRS is the same as SAMPLEHRS. If not, then
