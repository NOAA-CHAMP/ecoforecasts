function [stns,RGN] = get_all_misst_stns(region,cfgfname)
%function [stns,RGN] = get_all_misst_stns(region,cfgfname)
%
...
% pixels associated with stations named in CFGFNAME. If CFGFNAME is not
% specified or is empty, indices from the file [REGION '.cfg'] are used.
%
% RETURNS struct STNS of station structs (each fieldname of STNS is a five
% character station code); and RGN struct with .MEANSST field, monthly and
% weekly climatologies .MCLIMSST and .WCLIMSST, and vectors of all .LONS and
% .LATS and grid size .DX (in degs). See HELP READ_MISST_REGION for further
% details on each of these RGN fields.
%
% Before return, STNS and RGNS are SAVEd (qv.) in a .MAT file in datapath.
%
% Last Saved Time-stamp: <Thu 2010-11-04 15:38:29  Lew.Gramer>
  ...
  if (~exist('cfgfname','var') || isempty(cfgfname))
    cfgfname = fullfile(datapath,[lower(region) '.cfg']);;
  end;
  ...
  if ( ~exist(cfgfname) )
    error('No configuration file found! "%s"',cfgname);
  end;

  rawcfgs = importdata(cfgfname);
  cfgs = rawcfgs.textdata;
  

  %asam_stns_9km.mat
  matfname = fullfile(datapath, sprintf('misst_%s_stns_%s.mat', region, cfgfname));
