function datetick2_5(varargin)
%DATETICK2_5 fixes an annoying bug when LEGEND or COLORBAR is present: Lew.Gramer@noaa.gov 
% As of 2016-09-05, it also allows 'keepticklabels' - which does *not* LINKPROP
%
% Last Saved Time-stamp: <Tue 2017-07-04 19:38:14 Eastern Daylight Time gramer>
%
...
if nargin==2 && isstruct(varargin{2}) && isfield(varargin{2},'Axes') && isscalar(varargin{2}.Axes)
    ...
    for n=1:length(dtd.kids)
        if strcmpi(get(dtd.kids(n),'type'),'axes') ...
              && ~strcmpi(get(dtd.kids(n),'tag'),'legend') ...
              && ~strcmpi(get(dtd.kids(n),'tag'),'colorbar') ...
            dtd.axes=[dtd.axes dtd.kids(n)];
        end
    end
    %link all the axes together
    if ( numel(dtd.axes) > 1 )
      dtd.hlink=linkprop(dtd.axes,{[dtd.ax 'lim'];[dtd.ax 'tick'];[dtd.ax 'ticklabel']});
