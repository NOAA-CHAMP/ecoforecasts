function [sst, LONS, LATS] = query_avhrr_weekly_subset(bbox_or_stn_or_stanm, yr, wk, dataset, region)
%function [sst, LONS, LATS] = query_avhrr_weekly_subset(bbox_or_stn_or_stanm, yr, wk, dataset, region)
%
% Download SST mean, anomaly or synoptic image and return SST field. If first
% arg BBOX_OR_STN_OR_STANM is CHAR, assume it is a SEAKEYS (or ICON) station 
% code (e.g., 'fwyf1'), or if a station struct (with .lon and .lat fields),
% choose bounding-box accordingly. Otherwise this arg should be a 4-element
% vector containing [minlon maxlon minlat maxlat].
%
% Last Saved Time-stamp: <Mon 2016-02-01 16:13:25 Eastern Standard Time gramer>
  ...
  if ( ~exist('bbox_or_stn_or_stanm', 'var') || isempty(bbox_or_stn_or_stanm) )
    % Default is to show the whole Florida Reef Tract
    bbox_or_stn_or_stanm = [ -84 -79 23 26 ];
  end;
  ...
  if ( isnumeric(bbox_or_stn_or_stanm) )
    if ( numel(bbox_or_stn_or_stanm) ~= 4 )
      error('Bounding box should be a 4-vector of double');
    end;
    bbox = bbox_or_stn_or_stanm;
    ...
    stn = get_station_from_station_name(bbox_or_stn_or_stanm);

    stn_x = round((maxlat - stn.lat) / dlat);
    stn_y = round((stn.lon - minlon) / dlon);
  ...
  bbox_or_stn_or_stanm=[]; clear bbox_or_stn_or_stanm;
