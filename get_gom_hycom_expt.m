function stns = get_gom_hycom_expt(stns_or_stnms,mindt,maxdt,rad,expt,vars,flds,interpMethod,baseurl)
%function stns = get_gom_hycom_expt(stns_or_stnms,mindt,maxdt,rad,expt,vars,flds,interpMethod,baseurl)
%
...
%
% Last Saved Time-stamp: <Fri 2018-08-31 17:11:36 Eastern Daylight Time gramer>
      ...
      try
        ...
        for vix = 1:length(vars)
          ...
          if ( isempty(dts) )
            ...
            %%%% HACK HACK HACK dtix = find(ismember(alldts,dts));
            dtix = find(ismember(alldts,dts));
            [dts,alldtix,dtix] = intersect(alldts,dts);
            if ( length(dtix) ~= ndts )
% %%%%%%%%%%
% %%%%%%%%%% HACK HACK HACK ?!?!???
% %%%%%%%%%%
...
%keyboard;
              close(nc); clear nc;
              error('Date count mismatch %d vs %d! URL "%s" has beg %s, end %s',length(dtix),ndts,url,datestr(dts(1)),datestr(dts(end)));
            end;
          end;
...
%keyboard;
              ...
              %DEBUG:
              if ( all(isnan(stations(ix).(fld).data(dtix,1))) ); keyboard; end;
              dat = []; clear dat;
