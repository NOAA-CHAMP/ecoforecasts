1;
  ...
  %q0s = linspace(-500,0,20);				% dissertation figure
  %q0s = linspace(-200,0,20);
  q0s = linspace(-250,0,20);				% new proposal figure

  %bets = [0.002,0.02,0.20];
  %bets = 2 .* logspace(-3,-1,20);			% dissertation figure
  bets = 5 .* logspace(-3,-2,20);			% new proposal figure
  %bets = logspace(-3,0,20);
  ...
  % ylim([0.002,0.2]);
  % zlim([0,1]);
  mindT = min([min(dTdthc_SS(:)), min(dTdthc_SU(:)), min(dTdthc_US(:)), min(dTdthc_UU(:))]);
  maxdT = max([max(dTdthc_SS(:)), max(dTdthc_SU(:)), max(dTdthc_US(:)), max(dTdthc_UU(:))]);
  ...
  txtix = round(length(q0s)/2);
  text(q0s(1),bets(txtix),dTdthc_SS(txtix,1),'SS');
  text(q0s(1),bets(txtix),dTdthc_US(txtix,1),'US');
  text(q0s(1),bets(txtix),dTdthc_SU(txtix,1),'SU');
  text(q0s(1),bets(txtix),dTdthc_UU(txtix,1),'UU');

  xlabel('Sea-surface flux Q_0 [Wm^-^2]');
  ylabel('Seafloor slope \beta');
  zlabel('Horizontal convective warming [Kd^-^1]');

  zlim([0,maxdT]);

  titlename('Sensitivity analysis: Horizontal convection vs. \beta, Q_0');

  print('-dpng',[mfilename,'.png']);
