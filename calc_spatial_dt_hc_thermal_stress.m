1;
%% SCRIPT calc_spatial_dt_hc_thermal_stress
%
...
%
% Last Saved Time-stamp: <Tue 2018-12-11 14:37:32 Eastern Standard Time gramer>
...
q0str = {'2010 cold snap', 'Normal winter', 'Normal summer','2014 bleaching'};
...
if ( ~exist('bath_res','var') || isempty(bath_res) )
...
ecofpath = get_ecoforecasts_path;

matbasename = fullfile(ecofpath,sprintf('FRT_depth_and_beta_%dm',bath_res));
if ( use_habitat_map )
...
if ( ~exist('lon','var') || ~exist('lat','var')  || ...
  ...
  if ( exist(matfname,'file') )
    ...
    % Sometimes, unaveraged bathymetry got processed differently before saving??
    if ( size(h,1) ~= numel(lon) )
      if ( size(h,2) ~= numel(lon) )
        error('Misshapen field! %s',matfname);
      end;
      disp('Transposing fields...');
      if exist('ang','var'); ang = ang'; end;
      bet = bet';
      h = h';
      hch = hch';
      hcrng = hcrng';
    ...
    elseif ( use_habitat_map )
      ...
      if ( bath_res > 60 )
        ...
        [x,rad] = read_hires_bathymetry_for_field(fld,true);
      end;
...
if ( ~isempty(lon_rmix) || ~isempty(lat_rmix) )
  ...
  if exist('ang','var'); ang(:,lat_rmix) = []; ang(lon_rmix,:) = []; end;
...
for qix=1:nq0s;
  ...
  Q0 = repmat(q0,size(h));
  % For warming, try to account for effects of bottom solar reflectivity
  % ("gamma") with a simple formula based on local seafloor depth
  if ( q0 > 0 )
    Q0 = Q0 .* (tanh(2.*h./3));
  end;
