...
%
% Last Saved Time-stamp: <Tue 2015-08-04 16:04:23 Eastern Daylight Time gramer>
        ...
        if ( ~isempty(wnd) )
          ...
          if ( isempty(stn.(fld).date) )
            ...
            [stn.(newfld).date, stn.(newfld).data] = ...
                window_func(stn.(fld).date, stn.(fld).data, fun, wnd, samplesecs);
...
function [n, per, wnd, fun, rest, samplesecs] = get_derived_formula(varname_frag)
%INTERNAL function [n, per, wnd, fun, rest, samplesecs] = get_derived_formula(varname_frag)
%
...
% a base variable name: results are meant to be passed to window_func (qv).
  ...
  if ( ~isempty(n) && isnumeric(n) && n > 0 )
    switch ( per ),
     ...
     case {'w', 'week', 'weeks'},
      wnd = n * 60.0 * 60.0 * 24.0 * 7.0;
     case {'d', 'day', 'days'},
      wnd = n * 60.0 * 60.0 * 24.0;
     case {'h', 'hour', 'hours'},
      wnd = n * 60.0 * 60.0;
