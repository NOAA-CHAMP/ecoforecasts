...
%
% Last Saved Time-stamp: <Tue 2017-04-18 12:46:52 Eastern Daylight Time gramer>
...
if ( ~doWaves || 1 )
  ...
  sites.hs = repmat(nan,[365,numel(sites.lons)]);
  sites.wu = repmat(nan,[365,numel(sites.lons)]);
  sites.wv = repmat(nan,[365,numel(sites.lons)]);
  sites.tp = repmat(nan,[365,numel(sites.lons)]);
...
%   for ix = 1:numel(sites.stnms)
%     fld = sites.stnms{ix};
%     stns.(fld).ww3_sigwavehgt_clim = nanmean(sites.hs(:,ix));
%   end;
...
for stnmix = 1:numel(cstnms)
  ...
  if ( ~isfield(stns.(stnm),resvar) || ~is_valid_ts(stns.(stnm).(resvar)) )
    ...
    if ( isfield(sites,'hs') && any(~isnan(sites.hs(:,stnmix))) )
      %DEBUG:      disp([stnm,' =? ',sites.stnms{stnmix}]);
      Hses(stnmix) = nanmean(sites.hs(:,stnmix));
